      ...
      text = ''
        ...
        while [[ "$#" -gt 0 ]]; do
            ...
            -s | --session)
                ...
                read -r -a ts_commands_array <<< "$TS_SESSION_COMMANDS"

                if [[ -z "$session_idx" || "$session_idx" -lt 0 || "$session_idx" -ge "''${#ts_commands_array[@]}" ]]; then
                    echo "Error: Invalid index. Please provide an index between 0 and ''$((''${#ts_commands_array[@]} - 1))."
                    exit 1
                fi

                session_cmd="''${ts_commands_array[$session_idx]}"
        ...
        get_pane_id() {
            ...
            init_pane_cache
            grep "^''${target_idx}:''${target_split}:" "$PANE_CACHE_FILE" | cut -d: -f3
        }
        ...
        set_pane_id() {
            ...
            grep -v "^''${target_idx}:''${target_split}:" "$PANE_CACHE_FILE" > "''${PANE_CACHE_FILE}.tmp" 2>/dev/null || true
            mv "''${PANE_CACHE_FILE}.tmp" "$PANE_CACHE_FILE"

            echo "''${target_idx}:''${target_split}:''${pane_id}" >> "$PANE_CACHE_FILE"
        }
        ...
        if [[ -n ''${TS_SEARCH_PATHS:-} ]]; then
            read -r -a ts_search_paths <<< "$TS_SEARCH_PATHS"
        else
            ts_search_paths=("$HOME" "$HOME/personal" "$HOME/personal/dev/env/.config")
        fi

        if [[ -n ''${TS_EXTRA_SEARCH_PATHS:-} ]]; then
            read -r -a extra_paths <<< "$TS_EXTRA_SEARCH_PATHS"
            ts_search_paths+=("''${extra_paths[@]}")
        fi

        find_dirs() {
            if [[ -n "''${TMUX:-}" ]]; then
                current_session=$(tmux display-message -p '#S')
                tmux list-sessions -F "[TMUX] #{session_name}" 2>/dev/null | grep -vFx "[TMUX] $current_session" || true
            else
